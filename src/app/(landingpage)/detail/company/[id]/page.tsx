...
async function getDetailCompany(id: string) {
  const data = await prisma.company.findFirst({
    where: { id },
    // include itu relasi database, jadi di bawah ini berelasi dengan Company
    include: {
      Companyoverview: true,
      CompanySocialMedia: true,
      CompanyTeam: true,
      // hanya ambil total job saja
      _count: {
        select: {
          Job: true,
        },
  ...
  // ambil image companynya dari supabase
  if (data?.Companyoverview[0].image) {
    imageUrl = await supabasePublicUrl(
      data.Companyoverview[0].image, // ambil image array ke 0
      "company" // bucketnya company
    );
  } else {
    imageUrl = "/images/company2.png"; // default image jika tidak memiliki
  }
...
const DetailCompanyPage: FC<DetailCompanyPageProps> = async ({ params }) => {
  ...
  return (
    <>
      {/* jika ada data & data.Companyoverview maka munculin */}
